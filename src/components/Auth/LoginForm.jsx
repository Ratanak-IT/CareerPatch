...
export default function LoginForm() {
  ...
  const { field, validate } = useZodForm(loginSchema, { email: "", password: "" });
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      reduxDispatch(setUser(meData.data));
      toast.success("Login successful");
      navigate(redirectTo, { replace: true });
    } catch (err) {
      const msg = err?.data?.message || err?.data?.error || err?.error || `Unauthorized (${err?.status || 401})`;
      setServerError(msg);
      toast.error(msg);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center
      ...
      <div className="flex w-full max-w-sm sm:max-w-md md:max-w-3xl lg:max-w-4xl
        ...
        <div className="hidden md:flex flex-col items-center justify-center relative
          ...
          <img src={LoginImage} alt="Login illustration" className="w-[70%] max-w-[200px] object-contain"/>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center
          ...
          <form onSubmit={onSubmit} noValidate className="w-full max-w-[340px]">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-8 text-[#043133] dark:text-white">
              Login
            </h1>
            ...
            <div className="flex flex-col gap-5">
              <FormField label="Email"    type="email"    height="h-11" autoComplete="email"            {...field("email")}/>
              <FormField label="Password" type="password" height="h-11" autoComplete="current-password" {...field("password")}/>
            </div>
