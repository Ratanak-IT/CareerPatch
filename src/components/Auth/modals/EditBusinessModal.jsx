  ...
  return (
    <div className="w-full max-w-md">
      ...
      "
        >
          <svg
            className="w-12 h-12 text-blue-400 mb-1.5"
            viewBox="0 0 24 24"
            fill="currentColor"
          >
...
export default function EditBusinessModal({
  ...
  const profileName =
    profileFile?.name ?? (form.profileImageUrl ? "Current photo" : null);
    ...
    try {
      ...
      if (coverFile) {
        ...
        if (upsertErr) {
          ...
          setSubmitError(
            `Cover saved to Cloudinary but couldn't update profile: ${upsertErr.message}`,
          );
        } else {
  ...
  const btnLabel = uploadingProfile
    ? "Uploading photo…"
    : uploadingCover
      ? "Uploading cover…"
      : saving
