...
function JobCard({ job }) {
  ...
  // The jobs API returns only { id, userId, title, ... } — no owner info
  // embedded. We must fetch /api/users/:userId to get companyName + avatar.
  // This endpoint is PUBLIC (no auth needed). 404 is handled in userApi.js
  // by returning { data: null } so the card gracefully shows fallback.
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-3 text-xs text-gray-400 dark:text-slate-500">
          ...
          <span className={`font-semibold ${
            ...
            status === "DRAFT" ? "text-yellow-500" : "text-gray-500"
          }`}>
            {status}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          >
            <img
              src={authorAvatar} alt={authorName}
          ...
          <button
            ...
            type="button"
          >
            {isAuthed ? "Apply Now" : "Login to Apply"}
          </button>
        </div>
      </div>
    </Link>
