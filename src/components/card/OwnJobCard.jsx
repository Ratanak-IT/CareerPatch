...
function EditJobModal({ job, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    budget: job?.budget || "",
    jobImages: Array.isArray(job?.jobImages) ? job.jobImages.map(i => i?.imageUrl || i) : [],
  });
  ...
  const handleSave = async () => {
    try {
      await updateJob({ id: job.id, ...form, budget: form.budget ? Number(form.budget) : undefined }).unwrap();
      onClose();
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input className="bg-slate-100 rounded-lg px-3.5 py-2.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-400" value={form.title} onChange={(e) => setForm(p => ({ ...p, title: e.target.value }))} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-gray-700">Description</label>
            <textarea className="bg-slate-100 rounded-lg px-3.5 py-2.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-400 resize-none" rows={4} value={form.description} onChange={(e) => setForm(p => ({ ...p, description: e.target.value }))} />
          </div>
          <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="number" className="bg-slate-100 rounded-lg px-3.5 py-2.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-400" value={form.budget} onChange={(e) => setForm(p => ({ ...p, budget: e.target.value }))} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-gray-700">Image URLs</label>
            <div className="flex gap-2">
              <input className="flex-1 bg-slate-100 rounded-lg px-3.5 py-2.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-400" placeholder="Paste image URL" value={imageInput} onChange={(e) => setImageInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); if (imageInput.trim()) { setForm(p => ({ ...p, jobImages: [...p.jobImages, imageInput.trim()] })); setImageInput(""); } } }} />
              <button onClick={() => { if (imageInput.trim()) { setForm(p => ({ ...p, jobImages: [...p.jobImages, imageInput.trim()] })); setImageInput(""); } }} className="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold">Add</button>
            </div>
          </div>
        </div>
        <div className="px-6 pb-6 flex justify-end gap-3">
          ...
          <button onClick={handleSave} disabled={isLoading} className="px-8 py-2.5 rounded-full bg-blue-500 hover:bg-blue-600 disabled:opacity-60 text-white text-sm font-semibold flex items-center gap-2">
