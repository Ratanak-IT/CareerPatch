...
function Avatar({ src, name, size = "w-10 h-10", textSize = "text-sm" }) {
  ...
  return (
    <div className={`${size} rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold ${textSize} shrink-0`}>
      {initial}
    </div>
  );
}
...
function FileRow({ url, label }) {
  ...
  return (
    <div className="flex items-center gap-3">
      {/* Icon */}
      <div className="w-10 h-10 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 flex flex-col items-center justify-center shrink-0">
        {pdf ? (
          <>
...
// ─── Applicant detail panel (RIGHT side) ──────────────────────────────────────
function ApplicantDetail({ app, onBack, onViewProfile }) {
  if (!app) {
    return (
      <div className="flex items-center justify-center text-gray-300 dark:text-gray-600 py-20 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
      ...
      {onBack && (
        <div className="xl:hidden px-5 pt-4 pb-2">
          <button onClick={onBack}
            className="flex items-center gap-1.5 text-sm text-blue-500 font-semibold">
      ...
      <button
        ...
        className="w-full flex items-center gap-3 px-6 pt-5 pb-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group text-left"
      >
        <Avatar
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors truncate">
            {app.full_name}
          </p>
          <p className="text-xs text-blue-500 dark:text-blue-400 mt-0.5">View profile →</p>
        </div>
        <svg className="w-4 h-4 text-gray-300 dark:text-gray-500 group-hover:text-blue-400 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="px-6 py-5 space-y-3">
        {hasCv && (
          <FileRow url={app.cv_url} label={`${app.full_name?.split(" ")[0] || "applicant"}_CV`} />
        )}
        {hasOverview && (
          <FileRow url={app.overview_url} label={`${app.full_name?.split(" ")[0] || "applicant"}_Overview`} />
        )}
        {!hasCv && !hasOverview && (
...
function ViewApplyTab({ jobs, loadingJobs }) {
  ...
  const handleViewProfile = (app) => {
    if (app?.applicant_id) {
      navigate(`/freelancers/${app.applicant_id}`);
    }
  };
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-5 items-start">
      ...
      {/* LEFT: job selector + applicant list */}
                ...
                >
                  {thumb ? (
                    ...
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
                      ...
                      <svg className={`w-5 h-5 ${active ? "text-white" : "text-gray-300 dark:text-gray-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* RIGHT: applicant detail — always visible on xl, shown below list on smaller */}
      <div className={`w-full flex-1 min-w-0 ${!showDetail && !selectedApp ? "xl:block hidden" : "block"}`}>
        <ApplicantDetail
          ...
          onViewProfile={() => handleViewProfile(selectedApp)}
        />
      </div>
...
function PublicJobCard({ job }) {
  ...
  return (
    <Link
      to={`/jobs/${jobId}`}
      className="group bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col w-full border border-gray-100 dark:border-gray-700 hover:-translate-y-1"
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <p className="text-gray-400 dark:text-gray-500 text-xs leading-relaxed mb-3 overflow-hidden"
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {tags.slice(0, 3).map((t) => (
            <span key={t} className="bg-[#1E88E5] text-white text-[10px] font-semibold px-2.5 py-0.5 rounded-full">{t}</span>
          ))}
        </div>
...
export default function ProfileBusinessView({
  ...
  const handleMessage = useCallback(() => {
    ...
    const recipientName  = user?.companyName || user?.fullName || "Business";
    const recipientAvatar = user?.profileImageUrl || null;
    navigate("/chat", {
      state: { recipientId: String(recipientId), recipientName, recipientAvatar },
    });
          ...
          <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-4">
            ...
            {!isOwner && (
              <button
                ...
                className="bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center gap-1.5 shrink-0">
                ...
                <button key={key} onClick={() => setTab(key)}
                  ...
                  {badge > 0 && (
                    <span className={`text-[10px] rounded-full px-1.5 py-0.5 font-bold ${
                      tab === key ? "bg-white/20 text-white" : "bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400"
                    }`}>{badge}</span>
                  )}
                </button>
              ))}
