          ...
          <div className="flex flex-col gap-3">
            {contactRows.map(({ icon, val }) => (
              <div key={val} className="flex items-start gap-2.5">
                ...
                <span className="text-xs text-gray-600 break-all leading-snug">
                  {val}
                </span>
              </div>
            ))}
...
export default function ProfileBusinessPage() {
  ...
  const { data: rawMyJobs, isLoading: jobsLoading } = useGetMyJobsQuery(
    undefined,
    { skip: !isAuthed },
  );
  const { data: rawServiceBookmarks, isLoading: serviceBookmarksLoading } =
    useGetServiceBookmarksQuery(undefined, { skip: !isAuthed });
  const { data: rawJobBookmarks, isLoading: jobBookmarksLoading } =
    useGetJobBookmarksQuery(undefined, { skip: !isAuthed });
  ...
  const serviceBookmarks = useMemo(
    () => extractArray(rawServiceBookmarks),
    [rawServiceBookmarks],
  );
  const jobBookmarks = useMemo(
    () => extractArray(rawJobBookmarks),
    [rawJobBookmarks],
  );
  const categories = useMemo(
    () => extractArray(rawCategories),
    [rawCategories],
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              {serviceBookmarksLoading ? (
                ...
                <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {serviceBookmarks.map((bm) => (
                    <BookmarkedServiceCard
                      key={bm?.id || bm?.service?.id}
                      bm={bm}
                    />
                  ))}
                </div>
      ...
      {postModalOpen && (
        <CreateJobModal onClose={() => setPostModalOpen(false)} />
      )}
    </div>
  );
}
